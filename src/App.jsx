...
export default function App() {
  ...
  const handleGenerate = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (results.length === 0) {
        alert("No conflict-free schedules found with these filters. Try relaxing some constraints.");
      }
    }, 50);
  }, [selectedCourses, instructorFilters, blockedSlots]);
  ...
  const handleResetFilters = useCallback(() => {
    if (window.confirm('Reset all time and day blocks?')) {
      setBlockedSlots([]);
      setBlockedDays([]); // Clear legacy if exists
      setBlockedHours([]); // Clear legacy if exists
    }
  }, []);
      ...
      <div className="app-container">
        ...
        <header className="app-header">
          ...
          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
            ...
            >
              {generating ? 'Processing...' : `Generate (${selectedCourses.length})`}
            </button>
          </div>
        </header>
